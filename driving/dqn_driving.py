...
TRAJ_LEN = 10           # how many timesteps in trajectory
...
class Agent():
    ...
    def __init__(self, state_size, latent_size, action_size, seed):
        ...
        self.state_0 = torch.FloatTensor([[0.0, 0.0, 0.0, 0.3]])
...
def train(agent,
    n_episodes=20000, max_t=10, eps_start=1.0, eps_end=0.05, eps_decay=0.999, savename="dqn.pth"):
    """Deep Q-Learning.
    ...
    modes = []                         # list containing driving styles from each episode
...
def test(agent,
    n_episodes=100, max_t=10):

    z = torch.FloatTensor([0.0]*agent.latent_size)
    for i_episode in range(1, n_episodes+1):
        state = env.reset()
        for t in range(max_t):
            action = agent.act(z, state, 0.0)
            env.render()
            next_state, reward, done, info = env.step(action)
            agent.step(state, action, reward, next_state, done, info)
            state = next_state
...
if __name__ == "__main__":
    env = Environment()
    agent = Agent(state_size=4, latent_size=8, action_size=3, seed=0)
    performance = train(agent, savename="models/driving_ours.pth")
    pickle.dump(performance, open("results/driving-ours2.pkl", "wb" ))

    # agent.qnetwork_local.load_state_dict(torch.load("models/driving_dqn.pth"))
    # test(agent)
